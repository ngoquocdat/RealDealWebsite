...
export default function ChatView({ roomId, roomMessages }: ChatRoomListProps) {
  ...
  const handleSend = (event: React.FormEvent) => {
    ...
    const newMessage = {
      ...
      title: title,
      text: input,
      date: generateRandomDate(new Date()).toLocaleDateString(),
      avatar: avatar,
    };
    setTitle("");
    setAgency(true);
    setAvatar(
      "https://th.bing.com/th/id/R.588e9f4570ef5ddee0ef2f0c3eb4d237?rik=VDy%2fh%2ffN3XJm6Q&amp;pid=ImgRaw&amp;r=0"
    );
          ...
          {messages.slice(0).map((message, index) => (
            <ListItem
              ...
              sx={{
                display: "flex",
                flexDirection:
                  message.isAgency === true ? "row-reverse" : "row",
